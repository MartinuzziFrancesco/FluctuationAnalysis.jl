@testitem "segmentation" begin
    ...
    __segment_views = FluctuationAnalysis.__segment_views
    profile = collect(1.0:10.0)

    @testset "non-overlapping forward segmentation" begin
        segments = __segment_views(profile, 5; overlap = false, bidirectional = false)
    ...
    @testset "bidirectional covers the remainder" begin
        segments = __segment_views(profile, 4; overlap = false, bidirectional = true)
    ...
    @testset "bidirectional doubles the count when the scale divides evenly" begin
        segments = __segment_views(profile, 5; overlap = false, bidirectional = true)
        @test length(segments) == 4   # two forward + two from the end (same coverage)
    end

    @testset "overlapping sliding window" begin
        segments = __segment_views(profile, 4; overlap = true)
    ...
    @testset "smallest scale" begin
        segments = __segment_views(profile, 2; overlap = false, bidirectional = false)
        @test length(segments) == 5
        @test collect(segments[1]) == [1.0, 2.0]
    end

    @testset "scale equal to the profile length yields one segment" begin
        segments = __segment_views(profile, 10; overlap = false, bidirectional = false)
        @test length(segments) == 1
        @test collect(segments[1]) == profile
        @test length(__segment_views(profile, 10; overlap = true)) == 1
    end
    ...
    @testset "validation" begin
        @test_throws ArgumentError __segment_views(profile, 1)
        @test_throws ArgumentError __segment_views(profile, 11)
    end
end
