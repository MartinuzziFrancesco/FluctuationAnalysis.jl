@testitem "fluctuation" begin
    ...
    @testset "segment covariance reduces to variance for equal segments" begin
        ...
        @test isapprox(
            FluctuationAnalysis.__segment_covariance(detrender, segment, segment),
            FluctuationAnalysis.__segment_variance(detrender, segment);
            atol = 1.0e-12,
        )
    end
    ...
    @testset "fluctuation curve matches per-scale evaluation" begin
        ...
        scales = [8, 16, 32]
        detrender = PolynomialDetrender(1)
        curve = FluctuationAnalysis.__fluctuation_curve(profile, scales, detrender)
        expected = [
            FluctuationAnalysis.__fluctuation_at_scale(profile, scale, detrender) for
                scale in scales
        ]
        @test curve == expected
    end
    ...
    @testset "overlapping segmentation changes the estimate but stays positive" begin
        ...
        detrender = PolynomialDetrender(1)
        disjoint = FluctuationAnalysis.__fluctuation_at_scale(profile, 32, detrender)
        overlapped = FluctuationAnalysis.__fluctuation_at_scale(
    ...
    @testset "scale below the detrender minimum is rejected" begin
        profile = collect(1.0:64.0)
        @test_throws ArgumentError FluctuationAnalysis.__fluctuation_curve(
