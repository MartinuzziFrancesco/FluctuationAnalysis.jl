@testitem "scales" begin
    ...
    @testset "scales are sorted, distinct, and bounded" begin
        scales = logarithmic_scales(
            1000; minimum_scale = 4, maximum_scale = 200, scale_count = 20
        )
    ...
    @testset "rounding collisions are deduplicated" begin
        ...
        scales = logarithmic_scales(
            1000; minimum_scale = 4, maximum_scale = 8, scale_count = 20
        )
        @test allunique(scales)
    ...
    @testset "endpoints are honoured" begin
        scales = logarithmic_scales(
            4096; minimum_scale = 8, maximum_scale = 512, scale_count = 12
        )
        @test first(scales) == 8
    ...
    @testset "base changes the spacing" begin
        coarse = logarithmic_scales(
            10_000; minimum_scale = 4, maximum_scale = 2000, base = 2.0
        )
        fine = logarithmic_scales(
            10_000; minimum_scale = 4, maximum_scale = 2000, base = 1.2
        )
    ...
    @testset "validation" begin
        ...
        @test_throws ArgumentError logarithmic_scales(
            1000; minimum_scale = 50, maximum_scale = 40
        )
